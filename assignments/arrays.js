...
//let lastCar = inventory.length(lastMake, lastModel){
 //   console.log(in)
//}

console.log(`Make: ${inventory[inventory.length - 1].car_make} Model:${inventory[inventory.length - 1].car_model}`);
//console logging --> calling the inventory to check for the last index's car_make, and model
...
//You are assigning values to a new array called carModels by sorting through the current array using two paramenters
...
//Assigned carYears array to an inventory ampping function that has a car parameter
    return car.car_year;
//When called, function will return the car's year.
})
...
//Creating new array called oldCars that is assigned values by filtering through the carYears array based on car_year
    return car_year < 2000;
//To find cars made before car_year 2000, return car_year < 2000
})
